...
function buildFallbackMemberSnapshot(
  snapshot: {
    nickname?: string;
    gender?: string;
    ageMonths?: number;
  ...
  const suffix = deviceId.slice(-6) || 'guest';
  return {
    nickname: snapshot?.nickname || `访客${suffix}`,
...
export async function ensureMemberForDevice(input: {
  ...
  if (!profiles.length) {
    const snapshot = buildFallbackMemberSnapshot(input.memberSnapshot, input.deviceId);
