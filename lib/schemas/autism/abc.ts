...
const ABC_DIMENSIONS = [
  {
    id: 'sensory',
    label: '感觉',
    questionIds: [6, 10, 21, 26, 34, 39, 44, 52, 57],
  },
  {
    id: 'relating',
    label: '交往',
    questionIds: [3, 7, 13, 17, 24, 25, 27, 28, 33, 38, 43, 47],
  },
  {
    id: 'movement',
    label: '运动',
    questionIds: [1, 5, 9, 12, 16, 22, 30, 35, 40, 51, 53, 54],
  },
  {
    id: 'language',
    label: '语言',
    questionIds: [4, 8, 11, 15, 18, 20, 29, 32, 37, 42, 46, 48, 56],
  },
  {
    id: 'self_help',
    label: '生活自理',
    questionIds: [2, 14, 19, 23, 31, 36, 41, 45, 49, 50, 55],
  },
] as const;

const ABC_QUESTION_BY_ID = new Map(ABC_QUESTIONS.map((question) => [question.id, question]));

function calculateDimensionScore(answers: number[], questionIds: readonly number[]) {
...
function calculateDimensionMaxScore(questionIds: readonly number[]) {
  return questionIds.reduce((sum, questionId) => {
    const question = ABC_QUESTION_BY_ID.get(questionId);
    if (!question) {
      return sum;
    }

    const maxOptionScore = question.options.reduce((max, option) => Math.max(max, option.score), 0);
...
export const ABC_Scale: ExecutableScaleDefinition = {
  ...
  calculateScore: (answers: number[]) => {
    ...
    const dimensions = ABC_DIMENSIONS.reduce<Record<string, { label: string; score: number; maxScore: number; questionIds: number[] }>>((result, dimension) => {
      result[dimension.id] = {
        label: dimension.label,
        score: calculateDimensionScore(answers, dimension.questionIds),
        maxScore: calculateDimensionMaxScore(dimension.questionIds),
    ...
    let details: {
      level: string;
      description: string;
      dimensions: Record<string, { label: string; score: number; maxScore: number; questionIds: number[] }>;
