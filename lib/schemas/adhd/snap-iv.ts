...
const SNAP_DIMENSION_THRESHOLDS = {
  inattention: 13 / 9,
  hyperactivity: 13 / 9,
  oppositionalDefiant: 8 / 8,
} as const;
...
export const SNAP_Scale: ExecutableScaleDefinition = {
  ...
  calculateScore: (answers: number[]) => {
    const safeAnswers = Array.from({ length: 26 }, (_, index) => answers[index] ?? 0);
    const inattentionRawScore = safeAnswers.slice(0, 9).reduce((sum, s) => sum + s, 0);
    const hyperactivityRawScore = safeAnswers.slice(9, 18).reduce((sum, s) => sum + s, 0);
    const oppositionalRawScore = safeAnswers.slice(18, 26).reduce((sum, s) => sum + s, 0);
    const inattentionAverage = inattentionRawScore / 9;
    const hyperactivityAverage = hyperactivityRawScore / 9;
    const oppositionalAverage = oppositionalRawScore / 8;
    const inattentionScore = roundToTwoDecimals(inattentionAverage);
    const hyperactivityScore = roundToTwoDecimals(hyperactivityAverage);
    const oppositionalScore = roundToTwoDecimals(oppositionalAverage);
    const totalScore = inattentionRawScore + hyperactivityRawScore + oppositionalRawScore;
    const scoreDisplay = [
      formatAverageScore(inattentionScore),
      formatAverageScore(hyperactivityScore),
      formatAverageScore(oppositionalScore),
    ].join(" / ");

    let conclusion: string;
    let detailsStr =
      `【注意力缺陷均分】${formatAverageScore(inattentionAverage)} / 3（原始分 ${inattentionRawScore}/27，>= ${formatAverageScore(SNAP_DIMENSION_THRESHOLDS.inattention)} 提示异常）\n` +
      `【多动/冲动均分】${formatAverageScore(hyperactivityAverage)} / 3（原始分 ${hyperactivityRawScore}/27，>= ${formatAverageScore(SNAP_DIMENSION_THRESHOLDS.hyperactivity)} 提示异常）\n` +
      `【对立违抗均分】${formatAverageScore(oppositionalAverage)} / 3（原始分 ${oppositionalRawScore}/24，>= ${formatAverageScore(SNAP_DIMENSION_THRESHOLDS.oppositionalDefiant)} 提示异常）\n\n`;

    if (
      inattentionAverage >= SNAP_DIMENSION_THRESHOLDS.inattention ||
      hyperactivityAverage >= SNAP_DIMENSION_THRESHOLDS.hyperactivity ||
      oppositionalAverage >= SNAP_DIMENSION_THRESHOLDS.oppositionalDefiant
    ) {
    ...
    return { 
      ...
      details: {
        ...
        totalScoreHint: "依次对应注意力缺陷、多动/冲动、对立违抗；各维度均分满分均为 3 分。",
        dimensions: {
          inattention: {
            label: "注意力缺陷",
