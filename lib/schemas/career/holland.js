...
exports.HOLLAND_Scale = void 0;
const HOLLAND_DIMENSION_LABELS = {
    R: "实际型",
    I: "研究型",
    A: "艺术型",
    S: "社会型",
...
const HOLLAND_CAREER_SUGGESTIONS = {
...
const binaryQuestionMeta = new Map();
const ratingQuestionMeta = new Map();
function createBinaryQuestion(id, trait, sectionLabel, text) {
...
function createSelfRatingQuestion(id, trait, label) {
...
function buildHollandQuestions() {
    const questions = [];
    let id = 1;
    Object.entries(PART_TWO_INTERESTS).forEach(([trait, items]) => {
        items.forEach((item) => questions.push(createBinaryQuestion(id++, trait, "兴趣活动", item)));
    });
    Object.entries(PART_THREE_ABILITIES).forEach(([trait, items]) => {
        items.forEach((item) => questions.push(createBinaryQuestion(id++, trait, "擅长活动", item)));
    });
    Object.entries(PART_FOUR_OCCUPATIONS).forEach(([trait, items]) => {
...
exports.HOLLAND_Scale = {
    ...
    calculateScore: (answers) => {
        ...
        const [primaryTrait, secondaryTrait, tertiaryTrait] = topThree;
