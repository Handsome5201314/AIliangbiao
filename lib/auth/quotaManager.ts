...
export class QuotaManager {
  ...
  private static lastGuestCleanupAt = 0;
  private static readonly GUEST_RETENTION_MS = 24 * 60 * 60 * 1000;
  private static readonly GUEST_CLEANUP_INTERVAL_MS = 60 * 60 * 1000;
  ...
  private static async cleanupExpiredGuests() {
    const now = Date.now();
    if (now - this.lastGuestCleanupAt < this.GUEST_CLEANUP_INTERVAL_MS) {
      return;
    }

    this.lastGuestCleanupAt = now;
    ...
    try {
      await prisma.user.deleteMany({
        where: {
          ...
          email: null,
          createdAt: {
            lt: new Date(now - this.GUEST_RETENTION_MS),
          },
        },
      });
    } catch (error) {
