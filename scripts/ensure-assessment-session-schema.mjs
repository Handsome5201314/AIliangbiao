...
async function main() {
  ...
  console.log('[schema] ensuring AssessmentSession public token index exists');
  await prisma.$executeRawUnsafe(`
    create unique index if not exists "AssessmentSession_publicToken_key"
    on "AssessmentSession" ("publicToken");
  `);
