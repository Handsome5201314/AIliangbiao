...
export default function DoctorDashboardPage() {
  ...
  const cards = [
    ...
    { title: '最近备注', value: data?.recentNotesCount ?? 0, icon: <StickyNote className="h-6 w-6" />, color: 'bg-amber-50 text-amber-700' },
    { title: '科研导出', value: data?.researchExportCount ?? 0, icon: <FlaskConical className="h-6 w-6" />, color: 'bg-emerald-50 text-emerald-700' },
  ];
  ...
  const quickActions = [
    ...
    { title: '新生儿病房', description: '面向病房随访多个宝宝，独立管理档案、三围记录表和三联生长图。', href: '/doctor/neonates', icon: <Baby className="h-5 w-5" />, tone: 'bg-rose-50 text-rose-700' },
    { title: '患者管理', description: '查看主责与协作患者档案、量表时间线、备注和共享状态。', href: '/doctor/patients', icon: <ClipboardList className="h-5 w-5" />, tone: 'bg-indigo-50 text-indigo-700' },
    { title: '团队协作', description: '维护团队成员，并在患者档案和新生儿病房之间建立共享协作关系。', href: '/doctor/team', icon: <Users className="h-5 w-5" />, tone: 'bg-cyan-50 text-cyan-700' },
    { title: '医生邀请', description: '继续使用现有直链量表邀请链路，快速生成二维码与分享链接。', href: '/doctor/invites', icon: <Link2 className="h-5 w-5" />, tone: 'bg-amber-50 text-amber-700' },
    { title: '维护个人资料', description: '更新医生个人资料、执业信息和审核相关信息。', href: '/doctor/profile', icon: <UserRound className="h-5 w-5" />, tone: 'bg-emerald-50 text-emerald-700' },
  ];
    ...
    <div className="space-y-6">
      ...
      <Card className="border-cyan-200 bg-cyan-50 p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-900">医生工作流已扩展到团队协作</h2>
        ...
        <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
          {quickActions.map((action) => (
            <Link
              key={action.href}
              href={action.href}
              className="group rounded-3xl border border-slate-200 bg-slate-50 p-5 transition-all hover:border-cyan-200 hover:bg-white hover:shadow-sm"
            >
