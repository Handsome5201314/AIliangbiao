...
const rejectSchema = z.object({
  ...
  reviewNotes: z.string().trim().min(1, "驳回时请填写审核原因").max(500, "审核备注不能超过 500 个字符"),
});
...
export async function POST(
  ...
  try {
    ...
    const payload = rejectSchema.parse(rawPayload);
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to reject knowledge review item" },
      { status: 500 }
    );
  }
}
