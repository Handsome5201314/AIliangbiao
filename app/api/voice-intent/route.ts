...
async function callSystemTextModel<T>(input: {
  ...
  const agentPreferred = await resolveAgentApiKeyByService('text');
  const api = agentPreferred || (await getSystemApiKeyByService('text'));
...
async function resolveTriageIntent(input: z.infer<typeof triageRequestSchema>) {
  ...
  try {
    const llmResult = await callSystemTextModel({
      systemPrompt: agentConfig.prompts.triageSystemPrompt,
