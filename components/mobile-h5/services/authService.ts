import type { AuthUser, DoctorProfile } from '@/components/mobile-h5/types';
...
export function getStoredH5DeviceId() {
  return localStorage.getItem(STORAGE_KEY_H5_DEVICE_ID) || undefined;
}

export function getOrCreateH5DeviceId() {
  let deviceId = getStoredH5DeviceId();
  if (!deviceId) {
...
export async function loginWithSms(
  ...
  const session = assertAuthPayload(
    await apiRequest<AuthResponse>('/api/auth/login-sms', {
      method: 'POST',
      body: JSON.stringify({ phone, code, guestSessionId: getStoredH5DeviceId() }),
    }),
  );
      ...
      const session = assertAuthPayload(
        await apiRequest<AuthResponse>('/api/auth/login', {
          ...
          body: JSON.stringify({
            ...
            password,
            guestSessionId: getStoredH5DeviceId(),
          }),
        }),
      );
...
export async function loginAsGuest(): Promise<{
  ...
  const deviceId = getOrCreateH5DeviceId();
  const session = assertAuthPayload(
    await apiRequest<AuthResponse>('/api/auth/guest', {
      method: 'POST',
